...
namespace whitespacese\Composer;
...
class MatomoCoreInstaller extends LibraryInstaller {
	...
	const MESSAGE_SENSITIVE = 'Warning! %s is an invalid Matomo install directory (from %s)!';
	...
	public function getInstallPath( PackageInterface $package ) {
		...
		if ( $this->composer->getPackage() ) {
			...
			if ( ! empty( $topExtra['matomo-install-dir'] ) ) {
				$installationDir = $topExtra['matomo-install-dir'];
		...
		if ( ! $installationDir && ! empty( $extra['matomo-install-dir'] ) ) {
			$installationDir = $extra['matomo-install-dir'];
		}
		if ( ! $installationDir ) {
			$installationDir = 'matomo'; // default path
		}
